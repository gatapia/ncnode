 ...
 */
ncnode.lib.processor = function(name, obj, docs) {
  this.name_ =  name.split('.')[0];
  this.obj_ =  obj;
  this.docs_ =  docs;
  this.class_ = new ncnode.lib.clazz(this.name_, this.getClassOverview_());
};
...
ncnode.lib.processor.prototype.process = function() {
  for (var i in this.obj_) {
    if (i.charAt(i.length - 1) === '_') { continue; } // Ignore privates

    var val = this.obj_[i];
    var isObject = i.charAt(0) === i.charAt(0).toUpperCase();
    var isFunction = !isObject && typeof(val) === 'function';
    if (isObject) {
      new ncnode.lib.processor(i, val, this.docs_).process();
    } else if (isFunction) {
...
ncnode.lib.processor.prototype.evalFunctionArgs_ = function(name, val) {
  ...
  goog.array.forEach(str.split(','), function(arg) {
    ...
    if (!arg) return;
    args.push(new ncnode.lib.type('*', arg));
  });
  return args;
};
