...
ncnode.lib.gencode.prototype.processJSFile_ = function(f) {
  if (f.indexOf('fs.js') < 0) return; // Lets start with just fs.js
  ...
  for (var i in ncnode.lib.gencode.REQUIRED_GLOBALS_) { delete ctx[i]; };

  new ncnode.lib.processor(f, exp, this.allDocs_).process();
};
